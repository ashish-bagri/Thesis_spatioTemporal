...
void IplImagePyramid::init(IplImageWrapper image, double scaleFactor)
{
	...
	if (scaleFactor > 1.01)
		nLevels = static_cast<std::size_t>(
				floor(log(std::min(image->width, image->height) - log(1.0f)) / log(scaleFactor)));
//if (1 > nLevels)
//	cerr << "EROR: nLevels: " << nLevels << " image size: " << image->width << "x" << image->height << " scaleFactor: " << scaleFactor << endl;
	...
	for (std::size_t i = 1; i < nLevels; ++i) {
		...
		double newScaleFactor = pow(double(scaleFactor), int(i));
		CvSize newSize = cvSize(static_cast<int>(cvRound(image->width / newScaleFactor)),
				static_cast<int>(cvRound(image->height / newScaleFactor)));
...
void IplImagePyramid::init(CvSize initSize, int depth, int nChannels, double scaleFactor)
{
	...
	if (scaleFactor > 1)
		nLevels = static_cast<std::size_t>(
				floor(log(std::min(initSize.width, initSize.height) - log(1.0f)) / log(scaleFactor)));
//if (1 > nLevels)
//	cerr << "EROR: nLevels: " << nLevels << " image size: " << initSize.width << "x" << initSize.height << " scaleFactor: " << scaleFactor << endl;
	...
	for (std::size_t i = 1; i < nLevels; ++i) {
		...
		// scale the image from the last level to the wished size
		double newScaleFactor = pow(double(scaleFactor), int(i));
		CvSize newSize = cvSize(static_cast<int>(cvRound(initSize.width / newScaleFactor)),
				static_cast<int>(cvRound(initSize.height / newScaleFactor)));
