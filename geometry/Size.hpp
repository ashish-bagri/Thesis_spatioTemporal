...
{ wd = static_cast<int>(cvRound(c * wd)); ht = static_cast<int>(cvRound(c * ht)); return *this; }
...
{
    assert(c != float(0));
    wd = static_cast<int>(cvRound(wd / c)); ht = static_cast<int>(cvRound(ht / c));
    return *this;
}
...
{ wd = static_cast<long>(cvRound(c * wd)); ht = static_cast<long>(cvRound(c * ht)); return *this; }
...
{
    assert(c != float(0));
    wd = static_cast<long>(cvRound(wd / c)); ht = static_cast<long>(cvRound(ht / c));
    return *this;
}
...
{
	return Size<int>(static_cast<int>(cvRound(getWidth())), static_cast<int>(cvRound(getHeight())));
}
...
{
	return Size<long>(static_cast<long>(cvRound(getWidth())), static_cast<long>(cvRound(getHeight())));
}
